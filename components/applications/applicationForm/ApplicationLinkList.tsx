    ...
    return (
        <div className="flex animate-in items-end gap-2 fade-in slide-in-from-left-1">
            ...
            <button
                ...
                className="mb-1 rounded p-2 text-red-500 transition-colors hover:bg-red-50"
        ...
        <section className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3">
                {fields.map((field, index) => (
                    <LinkItem key={field.id} index={index} name={name} onRemove={remove} error={arrayError?.[index]?.url} />
                ))}
            </div>
