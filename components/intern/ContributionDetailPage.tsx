...
const ContributionDetailPage = ({ application, genres, links, zipcodes }: Props): ReactElement => {
    const searchParams = useSearchParams();
    const backHref = withSearchParams('/intern', searchParams);
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="overflow-hidden rounded-md border border-black bg-white shadow-sm">
                <ContributionDetails application={application} genres={genres} links={links} zipcodes={zipcodes} />
            </div>
        </div>
    );
};
