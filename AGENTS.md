...
Organizer-initiated `festival@` mail is Intern **Mails schreiben** (`/intern/mails`), not a CLI. Automatic mails (Bewerbung, Workshop opt-in, Helfi) stay in `lib/mail/`.
...
lib/
  ...
  mailMerge/               ← Mails schreiben / mail merge (variables, slot lines, simple HTML)
...
- **Turbopack HMR in Docker:** `CHOKIDAR_*` is ignored. `WATCHPACK_POLLING` covers the route-file watcher. Do **not** set `watchOptions.pollIntervalMs` on Next 16.2 — it switches Turbopack to PollWatcher and ordinary edits (labels, copy) do not appear until `app` restarts. New files that 404 with `Module not found` still need `docker compose restart app`.
